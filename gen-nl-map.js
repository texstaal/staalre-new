...
const SRC = 'https://raw.githubusercontent.com/georgique/world-geojson/develop/countries/netherlands.json';
const W = 560;           // target viewBox width
const PAD = 16;          // padding inside the viewBox

// logistics hubs (lat, lon) + a short note shown on hover/tap
const HUBS = [
  { name: 'Rotterdam', lat: 51.95, lon: 4.14, note: 'Europe’s largest seaport — deep-sea containers and port-edge distribution.' },
  { name: 'Schiphol / Amsterdam', lat: 52.31, lon: 4.76, note: 'Air cargo and high-value, time-critical logistics around the capital.' },
  { name: 'Tilburg–Waalwijk', lat: 51.56, lon: 5.05, note: 'Central Brabant: XXL warehousing and the e-commerce fulfilment cluster.' },
...
(async () => {
  ...
  const f = j.features ? j.features[0] : j;
  ...
  euPolys.forEach(poly => poly.forEach((ring, ri) => {
    ...
    if (ring.length < 12) return;       // skip tiny specks
    ring.forEach((p, i) => {
